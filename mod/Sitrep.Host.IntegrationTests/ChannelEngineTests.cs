...
namespace Sitrep.Host.IntegrationTests
{
    ...
    public class ChannelEngineTests
    {
        ...
        /// <summary>
        /// Spanning regression for the terminal-epsilon invariant flagged by
        /// adversarial review before landing Gonogo.Kos's terminal fixes:
        /// <c>Gonogo.Kos.KosTerminalManager.UtEpsilon</c> and this class's own
        /// <c>PublishUtToleranceSeconds</c> now both derive from the single
        /// <see cref="EnginePublishTolerance.Seconds"/> constant (see its doc
        /// comment) SPECIFICALLY so that a same-tick frame stamped
        /// <c>clock.Now() + UtEpsilon</c> survives <see cref="ProcessPublish"/>'s
        /// stale-ut clamp instead of being clamped straight back to
        /// <c>clock.Now()</c> and colliding with the frame it was meant to
        /// stay ahead of. Fix #1's whole point was strictly-increasing
        /// <c>ValidAt</c> stamps on the terminal's diff stream; without this,
        /// two same-tick frames silently collapse and the original garble
        /// returns with no test catching it.
        ///
        /// This proves the invariant end-to-end at the real
        /// <see cref="ChannelEngine"/>/<see cref="ProcessPublish"/> boundary,
        /// using the ACTUAL shared constant rather than a hand-picked test
        /// value: two publishes processed while <c>_clock.Now()</c> is the
        /// SAME value (the second bumped by exactly
        /// <see cref="EnginePublishTolerance.Seconds"/>, mirroring
        /// <c>KosTerminalManager.NextUt</c>'s same-tick collision branch) —
        /// enqueued back-to-back, before any intervening Tick job, exactly
        /// the same-tick window <c>KosTerminalManager.Poll</c> can produce
        /// several frames within — must both reach the wire as two DISTINCT
        /// <see cref="Delivery.ReliableOrdered"/> frames, in order, with
        /// their <c>Meta.ValidAt</c> stamps left un-clamped and strictly
        /// increasing, never coalesced into one.
        /// </summary>
        [Fact]
        public async Task AnEpsilonBumpedSameTickPublishSurvivesTheStaleUtClampAndDeliversTwoDistinctFrames()
        {
            using var engine = new ChannelEngine("ws://127.0.0.1:0", networkDelaySeconds: 0);
            var uplink = new EpsilonPublishTestUplink();
            engine.RegisterUplink(uplink);
            engine.Start();
            try
            {
                await using var client = await TestClient.ConnectAsync(engine.BoundPort, Timeout);
                await SubscribeAsync(client, EpsilonPublishTestUplink.Topic, Timeout);

                // Establish _clock.Now() == 100.0.
                engine.TickAndWait(100.0, null, Timeout);

                // Two publishes enqueued before any further Tick job -- both
                // are processed by ProcessPublish while _clock.Now() is
                // STILL 100.0, exactly the window a real KosTerminalManager
                // same-tick burst produces. The second's ut is the terminal
                // manager's own same-tick-collision bump: last + UtEpsilon.
                uplink.Publisher!.Publish("frame-1", 100.0);
                uplink.Publisher!.Publish("frame-2", 100.0 + EnginePublishTolerance.Seconds);

                // Advance the clock past both (networkDelaySeconds: 0, so
                // DelayRole.Delayed reveals immediately) to flush them.
                engine.TickAndWait(100.0 + EnginePublishTolerance.Seconds, null, Timeout);

                var frames = await DrainAllStreamDataAsync(client, TimeSpan.FromMilliseconds(500));
                var seen = frames.FindAll(f => f.Topic == EpsilonPublishTestUplink.Topic);

                // The whole point of the invariant: frame-2's ut was NOT
                // clamped down to clock.Now() (100.0) -- both frames survive
                // as distinct deliveries, in publish order, with strictly
                // increasing ValidAt.
                Assert.Equal(2, seen.Count);
                Assert.Equal("frame-1", seen[0].Payload);
                Assert.Equal("frame-2", seen[1].Payload);
                Assert.Equal(100.0, seen[0].Meta.ValidAt);
                Assert.Equal(100.0 + EnginePublishTolerance.Seconds, seen[1].Meta.ValidAt);
                Assert.True(seen[1].Meta.ValidAt > seen[0].Meta.ValidAt);
            }
            finally
            {
                engine.Stop();
        ...
        /// <summary>
        /// The KSP-independent stand-in this file's own
        /// <see cref="AnEpsilonBumpedSameTickPublishSurvivesTheStaleUtClampAndDeliversTwoDistinctFrames"/>
        /// drives directly — a single <see cref="Delivery.ReliableOrdered"/> +
        /// <see cref="DelayRole.Delayed"/> channel exposed as a raw
        /// <see cref="IChannelPublisher"/>, same shape as
        /// <c>Gonogo.Kos.KosTerminalManager</c>'s real
        /// <c>kos.terminal.&lt;coreId&gt;</c> channel (see
        /// <c>KosChannels.TerminalSubTopic</c>).
        /// </summary>
        private sealed class EpsilonPublishTestUplink : ISitrepUplink
        {
            public const string Topic = "epsilon.test";

            public IChannelPublisher? Publisher { get; private set; }

            public UplinkManifest Manifest { get; } = new UplinkManifest
            {
                Id = "test-epsilon-publish",
                Version = "1.0.0",
