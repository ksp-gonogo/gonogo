...
export interface CommsDelay
{
	...
	*/
	oneWaySeconds?: Value<"s">;
	/**
	* How fast `CommsDelay.oneWaySeconds` is changing, in seconds of delay per
	* second of universal time. Positive while the craft recedes.
	*
	* Published rather than left to be differenced, because differencing it
	* downstream cannot be done honestly. A consumer sees a scalar and has no way
	* to tell motion from a REROUTE: the moment the route changes hop for hop the
	* total jumps discontinuously, and the jump divided by the interval is a rate
	* of nothing. The producer holds the ordered hop list this total was summed
	* from, so it can compare route identity between ticks and decline; and it
	* observes on every physics tick rather than at whatever rate the channel is
	* sampled at or a relayed station receives it.
	*
	* **null** whenever there is no honest rate to report, which is every case
	* that is not two consecutive observations over one unchanged route: the first
	* observation of a route, the tick a reroute lands on, a non-advancing or
	* rewound clock, and every case where `CommsDelay.oneWaySeconds` is itself
	* null. It is never 0 as a stand-in for "unknown"; a 0 means measured, and a
	* craft holding station really does have a rate of zero.
	*
	* A SECANT over the last tick interval, not an instantaneous derivative, so
	* under time warp it is the mean rate across an interval that may be long.
	* Consuming it is a first-order carry and diverges as the route's geometry
	* curves; a consumer owns that horizon.
	*/
	oneWaySecondsRate?: Value<"1">;
	/**
	* The speed light travels at in THIS save, m/s: the real constant scaled by
	* whatever factor the delay model is configured with. A save left at clean
	* realism reports 299792458.
	*
	* Here so a consumer can convert a distance to a light-time without assuming
	* the real constant. `comms.path` publishes each hop's distance in metres and
	* no per-hop time, so anything wanting one has to divide, and dividing by the
	* textbook speed of light is simply wrong on any save that scales it. The
	* EFFECTIVE speed rather than the scale factor, because the scale is a
	* configuration knob whose meaning a consumer would have to be taught, and
	* this is a physical quantity in the units of the distances it divides.
	*
	* **null** when nothing says: no delay configuration reached the computation,
	* or the configured scale is not a usable positive number. Populated even
	* where `CommsDelay.oneWaySeconds` is null or zero, since a save's light speed
	* is knowable while the craft has no route home and while the delay feature is
	* switched off.
	*/
	lightSpeedMetresPerSecond?: Value<"m/s">;
