...
pnpm exec gonogo-uplink docs --no-assets  # README.md + gonogo-uplink.json only
...
**`docs --no-assets` writes the first two and leaves `docs/assets/` alone.** Use
it when the prose has to move but the pictures should come from somewhere else:
adding or removing a scene changes the page's image links, and if your committed
renders come from a CI runner, re-rendering all of them on your own machine
commits a different OS's rasterisation for every picture you did not touch. It
still opens a browser, because your registrations only exist once your client has
run, but it mounts no scene. The images a new scene links are missing until the
next full `docs` run, and `docs --check` names them as missing rather than
passing.
...
Outside this repo: install `@ksp-gonogo/uplink-tools` as a **devDependency** and
name it:
