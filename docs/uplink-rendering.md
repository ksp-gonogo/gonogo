...
**`docs --no-assets` regenerates the page without touching `docs/assets/`.** It
still renders, because the README is built from what the renders found, but into
a scratch directory, so the committed images stay exactly as they were. Reach for
it when a change moves the prose (a scene added, a field renamed) on a machine
whose images you do not commit, because they are rasterised on a different OS
from the one that renders your published set.
