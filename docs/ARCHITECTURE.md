...
- **kOS integration** rides the Gonogo mod's sitrep stream: script dispatch over the `kos.run` Uplink command and CPU discovery over the `kos.processors` channel. The kOS Uplink lives in the gonogo-uplinks repository, and degrades gracefully when no stream is mounted.
