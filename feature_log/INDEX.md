...
- 2026-05-14 — [Power Systems — flow / nominalFlow + producers/consumers widget](2026-05-14-power-systems.md) — uncommitted — ⏳ pending (Telemachus fork: r.resourceFor[fid] extended with signed flow + nominalFlow from ModuleDeployableSolarPanel / Generator / ResourceConverter / ResourceHarvester / Engines; client: new PowerSystems widget aggregating producers vs consumers per resource with cycle action + compact mode; ShipMap parts gain a subtle EC producer/consumer ring; closes the 2026-05-14 audit follow-up; needs KSP restart + flight covering each module type to promote)
- 2026-05-14 — [tar.availableVessels — native vessel listing, retires kOS feed](2026-05-14-tar-available-vessels.md) — uncommitted — ⏳ pending (Telemachus fork: new `tar.availableVessels` handler returning `[{ index, name, type, situation, body, position }]` walking FlightGlobals.Vessels with default Flag/EVA/Debris/Unknown + active-vessel exclusion; client schema + meta; TargetPicker fully rewritten on the native path, kOS dependency dropped; deletes `vesselListScript.ts` and `setTargetScript.ts`; Telemachus.dll rebuilt + synced — needs KSP restart to load)
...
- 2026-05-14 — [Ship Map — seq-driven `v.topology` refetch](2026-05-14-ship-map-seq-driven-refetch.md) — uncommitted — ⏳ pending (new `useTopology` hook in @gonogo/data; subscribes to `v.topologySeq` continuously, briefly re-subscribes to `v.topology` only on bump; cuts steady-state wire bytes from ~5KB/s on a 9-part vessel — ~120KB/s on a 200-part station — to a small int per tick; needs live browser-side WS-frame inspection to promote)
...
- 2026-05-14 — [Ship Map widget — re-wired onto Telemachus v.topology](2026-05-14-ship-map-on-telemachus-topology.md) — `cb5d74c` — ⏳ pending (kOS pipeline dropped; subscribes to `v.topology` + dynamic `r.resourceFor[fid]` / `therm.part[fid]` via new `usePartsLive`; prefab bounds replace the mass-cubed-root sizing heuristic; tag chip-row dropped — no KSP-native equivalent; **fork API curl-validated live 2026-05-14**: topology + seq + r.resourceFor + therm.part all confirmed including decouple-bumps-seq and crash-bumps-seq; widget rendering not yet visually verified — superseded inline performance optimisation by [seq-driven refetch](2026-05-14-ship-map-seq-driven-refetch.md))
