...
- 2026-05-14 — [Power Systems — flow / nominalFlow + producers/consumers widget](2026-05-14-power-systems.md) — `db4c29a` — ⏳ pending (Telemachus fork: r.resourceFor[fid] extended with signed flow + nominalFlow from ModuleDeployableSolarPanel / Generator / ResourceConverter / ResourceHarvester / Engines; client: new PowerSystems widget aggregating producers vs consumers per resource; 2026-05-15 live pass validated solar / RTG / engine / fuel-tank dispatch cleanly; engine flow units bug found (per-physics-frame instead of per-second) and fixed in source — DLL rebuild + KSP restart needed for live re-verify; ISRU + drill not exercised on the test craft; consumer-coverage gap identified for ModuleDataTransmitter / TelemachusPowerDrain / ModuleCommand / ModuleReactionWheel / ModuleLight / ModuleAlternator — fork v2 expansion item)
...
- 2026-05-14 — [Ship Map — seq-driven `v.topology` refetch](2026-05-14-ship-map-seq-driven-refetch.md) — `61be606` — ⏳ pending (new `useTopology` hook in @gonogo/data; subscribes to `v.topologySeq` continuously, briefly re-subscribes to `v.topology` only on bump; cuts steady-state wire bytes from ~5KB/s on a 9-part vessel — ~120KB/s on a 200-part station — to a small int per tick; widget rendered cleanly through many high-frequency seq bumps in the 2026-05-15 live pass, so the React layer is structurally sound; DevTools WS-frame inspection still pending to directly confirm topology frames only flow after a bump)
