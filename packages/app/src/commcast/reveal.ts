 ...
 * of one. A group is therefore sent as one message per target, each with its
 * own separation, not as one message naming several.
 */
export function roundTripFor(msg: CommsMessage): RoundTrip | null {
  ...
  // A separation cannot speak for several recipients, so rather than pick one
  // of them this declines to state a schedule at all.
