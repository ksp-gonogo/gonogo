...
describe("seedKspHostDefaults", () => {
  it("seeds kerbcast and the Sitrep stream from a LAN KSP_HOST", async () => {
  ...
  it("maps container-internal hosts to localhost for browser-dialled sources", async () => {
  ...
  it("never overrides a user-saved config", async () => {
    ...
    // Like the kerbcast assertion above: this proves the seed itself was
    // skipped (`configStore.isStored()` saw the raw write and bailed), not
    // that the raw write hydrated the runtime's cached snapshot —
    // `sitrepRuntime` only reflects a saved value once something reads
    // through its own API, not a raw `localStorage.setItem`.
    expect(getSitrepHostConfig().host).toBe("localhost");
  });
