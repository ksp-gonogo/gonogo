 ...
 * harness.
 *
 * ## The two halves resolve from DIFFERENT places, and that is the whole care
 *
 * This used to be one loop over three siblings, correct only because all three
 * shipped from `@ksp-gonogo/ui-kit`. They no longer do:
 *
 *   - `render-probe` is a sibling of THIS file, in `@ksp-gonogo/uplink-tools`
 *   - the kit and its `/testing` subpath are a different package entirely, and
 *     must be the copy the AUTHOR installed, for the same reason `react` is
 *
 * Left as `join(HERE, ...)`, the kit's two entries would simply not be found,
 * `existsSync` would skip them, and the page would silently carry two kits.
 * That is the failure the paragraph above is about, and it reports nothing.
 * `page.test.ts` fails if either pin goes missing.
 */
function oneKitPerPage(dir: string): Record<string, string> {
  ...
  // Theirs. `require.resolve` cannot answer this: the kit's `exports` declares
  // only the `import` condition, and no `./package.json` at all, so a CJS
  // resolver refuses outright and the throw would be swallowed into "no pin".
  // Walking `node_modules` up from the Uplink is what a bundler does and is the
  // one layout pnpm's symlink farm and npm's flat tree agree on.
  const kit = nearestPackageDir(dir, "@ksp-gonogo/ui-kit");
  if (kit) {
...
/** The nearest installed copy of `pkg`, walking up from `dir`. */
function nearestPackageDir(dir: string, pkg: string): string | undefined {
  let at = resolve(dir);
  for (;;) {
...
export function generateEntry(
  ...
  const lines = [
    "// GENERATED by @ksp-gonogo/uplink-tools. Never committed, never edited.",
    'import { installRenderProbe } from "@ksp-gonogo/uplink-tools/render-probe";',
    "",
