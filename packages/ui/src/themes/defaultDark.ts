 ...
 * shades) is deliberately deferred to a later commit so a Phase 2 sweep can
 * be a pure structural refactor with no visual diff.
 *
 * Typography sizes are emitted as CSS-variable strings so the responsive
 * overrides in `packages/app/src/styles/global.css` (coarse-pointer bumps
 * etc.) keep working when consumers read from the theme instead of the raw
 * variable.
 */
export const defaultDarkTheme: GonogoTheme = {
  colors: {
    text: {
