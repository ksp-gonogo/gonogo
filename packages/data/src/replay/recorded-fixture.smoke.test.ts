 ...
 * flight, not a synthesized fixture. The recording lives outside the repo
 * (`local_docs/` is gitignored) so the test gates on file existence and
 * skips cleanly in CI / fresh clones — when the file IS present locally,
 * it asserts the fixture round-trips through `isFlightFixture` and that
...
const describeIfPresent = existsSync(FIXTURE_PATH) ? describe : describe.skip;

describeIfPresent(
  "FlightReplayDataSource — recorded launch_to_apoapsis_10000",
  () => {
