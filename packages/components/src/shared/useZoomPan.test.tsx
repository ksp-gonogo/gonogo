...
function setup() {
  const el = makeInteractionElement();
...
describe("useZoomPan", () => {
  it("pans with a single pointer once movement exceeds the threshold", () => {
    ...
    act(() => {
      onPointerDown(
        pointerEvent({ pointerId: 1, clientX: 50, clientY: 50 }, el),
      );
    });
    act(() => {
      onPointerMove(
        pointerEvent({ pointerId: 1, clientX: 80, clientY: 70 }, el),
      );
    });
    act(() => {
      onPointerUp(pointerEvent({ pointerId: 1, clientX: 80, clientY: 70 }, el));
    });
  ...
  it("ignores tiny pointer movements (under panThreshold)", () => {
    ...
    act(() => {
      onPointerMove(
        pointerEvent({ pointerId: 1, clientX: 51, clientY: 51 }, el),
      );
    });
  ...
  it("zooms with a two-pointer pinch", () => {
    ...
    act(() => {
      onPointerDown(
        pointerEvent({ pointerId: 1, clientX: 80, clientY: 50 }, el),
      );
      onPointerDown(
    ...
    act(() => {
      ...
      onPointerMove(
        pointerEvent({ pointerId: 2, clientX: 160, clientY: 50 }, el),
      );
    });
  ...
  it("reset returns to identity transform", () => {
    ...
    act(() => {
      onPointerDown(pointerEvent({ pointerId: 1, clientX: 0, clientY: 0 }, el));
    });
    act(() => {
      onPointerMove(
        pointerEvent({ pointerId: 1, clientX: 50, clientY: 50 }, el),
