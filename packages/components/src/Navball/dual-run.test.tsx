 ...
 * (the DEFAULT config's root-part frame `n.heading2`/`n.pitch2`/`n.roll2`
 * -> `vessel.attitude.*RootFrame`; `f.sasEnabled` ->
...
describe("Navball — behavior-preservation golden dual-run (delay=0)", () => {
  it("renders IDENTICAL markup off the stream as off the legacy DataSource for the same attitude/control state", async () => {
    ...
    act(() => {
      ...
      // CoM fields are emitted too (present on the real wire) so the payload
      // matches the contract shape, but the default render reads *RootFrame.
