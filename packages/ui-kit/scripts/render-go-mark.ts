#!/usr/bin/env tsx
/**
 * Render the go-mark token sheets (Saga #551) through a real Chromium page.
 *
 * Run: `node scripts/render-go-mark.ts -- --out <dir> --tag <before|after>`
 * Output: `<dir>/<NN>-<tag>-<sheet id>.png`, one per sheet in `SHEETS`.
 *
 * The tag and the output directory are arguments rather than constants
 * because the point of the script is a PAIR: the same sheets, the same
 * viewport and the same scale factor, run once on each side of the token
 * move. A before taken any other way is a picture of something else.
 */
...
import { SHEETS } from "./goMarkScenarios.ts";

const HERE = dirname(fileURLToPath(import.meta.url));
const ENTRY = join(HERE, "render-go-mark.entry.tsx");
...
async function main(): Promise<void> {
  const outDir = resolve(
    arg("out", resolve(HERE, "../../../local_docs/renders/go-mark")),
  );
...
<script type="module">${js}</script></body></html>`;
  const page = join(tmpdir(), `gonogo-go-mark-${process.pid}.html`);
