#!/usr/bin/env tsx
/**
 * Render the instrument reckoning sheets through a real Chromium page.
 *
 * Run: `pnpm --filter @ksp-gonogo/ui-kit render:instrument-readings -- --out <dir> --tag <before|after>`
 ...
 * same scale factor, run once on each side of a change to the four instruments. A before
...
import { SHEETS } from "./instrumentReadingScenarios.ts";

const HERE = dirname(fileURLToPath(import.meta.url));
const ENTRY = join(HERE, "render-instrument-readings.entry.tsx");
...
async function main(): Promise<void> {
  const outDir = resolve(
    arg(
      "out",
      resolve(HERE, "../../../local_docs/renders/instrument-readings"),
    ),
  );
...
<script type="module">${js}</script></body></html>`;
  const page = join(tmpdir(), `gonogo-instrument-readings-${process.pid}.html`);
  ...
  try {
    ...
    for (const [i, sheet] of SHEETS.entries()) {
      const bare = tag === "before" ? "&bare=1" : "";
      const url = `${pathToFileURL(page).toString()}?sheet=${sheet.id}${bare}`;
