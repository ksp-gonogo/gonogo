...
- `.github/workflows/release.yml` is the ONLY workflow that can publish packages: npm's trusted publishers and nuget.org's policy both name this file. `publish-packages` sends `ui-kit` and `sitrep-sdk` to npm; `publish-nuget` sends `KspGonogo.Sitrep.Contract` to nuget.org, packing once and pushing the same file it gated and probed (`nuget-contract-package-gate.mjs`, then `nuget-extraction-probe.mjs --nupkg` building `GonogoProbeUplink` (`scripts/nuget-probe-uplink/`, an Uplink kept here only to be probed, since every real one leaves) against it outside the repo, plus a planted gap that must fail). OIDC only, no API key; it needs the `NUGET_USER` secret (the nuget.org profile name that owns the policy). Dispatch with `-f packages_only=true -f dry_run=true` to run everything but the login and push. The NuGet version is the contract's `Major.Minor.0`, and an unchanged version is skipped on the version alone, so a TestSupport or `Sitrep.Core` change needs a contract Minor bump to ship. `ci.yml`'s `mod` job runs the same pack, gate and probe on every push
