...
kOS lives entirely in its own Uplink, `uplinks/kos/` in the gonogo-uplinks repository (C# mod plus the client TS it ships). Nothing kOS-specific remains in this repo beyond the shared `kos.*` topic knowledge in `sitrep-sdk`. It rides the Sitrep telemetry stream, there is no separate proxy process, and kOS features degrade gracefully when no stream is mounted.
...
- **`KosDataSource`** (`uplinks/kos/client/src/dataSource/kos.ts` in gonogo-uplinks): a plain RPC client. It holds no persistent socket and carries no subscribable data keys of its own, so it is registered via `registerUplinkHandle("kos", kosSource)` rather than `registerDataSource`, and never appears in the generic Data Sources panel. kOS health surfaces mod-side through `ISitrepUplink.Health()` (`KosHealth`); the `IUplinkHealthReporter` companion that used to carry it is retired, and `Sitrep.Host.Tests/MandatoryHealthContractTests.cs` fails if it comes back.
...
- `.github/workflows/release.yml` is the ONLY workflow that can publish packages: npm's trusted publishers and nuget.org's policy both name this file. `publish-packages` sends `ui-kit` and `sitrep-sdk` to npm; `publish-nuget` sends `KspGonogo.Sitrep.Contract` to nuget.org, packing once (with both determinism flags set) and pushing the same file it gated and probed (`nuget-contract-package-gate.mjs`, then `nuget-extraction-probe.mjs --nupkg` building `GonogoProbeUplink` (`scripts/nuget-probe-uplink/`, an Uplink kept here only to be probed, since every real one leaves) against it outside the repo, plus a planted gap that must fail). OIDC only, no API key; it needs the `NUGET_USER` secret (the nuget.org profile name that owns the policy). Dispatch with `-f packages_only=true -f dry_run=true` to run everything but the login and push. The NuGet version is `Major.Minor.PackagePatch`: `Major.Minor` is the contract's own (`ContractVersion.cs`), `PackagePatch` is the package's own (`Sitrep.Contract.Package.csproj`), for a `Sitrep.Contract.TestSupport` or `Sitrep.Core` change with no contract move. When the version is already on nuget.org, `nuget-fossil-check.mjs` downloads that published copy and compares it against the fresh pack with the five build-identity regions (PE timestamp, debug-directory timestamps, PDB id, PDB checksum, module MVID) normalised out; a real difference fails the release asking for a `PackagePatch` bump rather than skipping silently. `ci.yml`'s `mod` job runs the same pack, gate and probe on every push
